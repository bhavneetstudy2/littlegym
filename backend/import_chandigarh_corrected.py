...
from app.utils.enums import AttendanceStatus, EnrollmentStatus, PlanType
...
def import_enrollment_metadata(db: Session):
    ...
    print("\n" + "="*70)
    print("STEP 1: Importing Enrollment Metadata from Excel")
    print("="*70)

    # Read Excel file
    df = pd.read_excel(EXCEL_FILE, sheet_name='Enrolled')
    print(f"Found {len(df)} enrolled students in Excel")
    ...
    for idx, row in df.iterrows():
        try:
            ...
            attended_classes = int(row.get('Attended Classes', 0)) if not pd.isna(row.get('Attended Classes')) else 0
            ...
            if not child:
                ...
                print(f"  ! Child not found: {enquiry_id} - {child_name}")
                continue

            # Update child with enquiry_id
            ...
            if enrollment:
                ...
                # Add duration to notes if available
                if duration:
                    enrollment.notes = f"Duration: {duration}"

                updated_enrollments += 1
                print(f"  ✓ Updated: {enquiry_id} - {child_name} ({batch_name}): {booked_classes} classes")
            else:
                print(f"  ! No enrollment found for: {enquiry_id} - {child_name}")
    ...
    print(f"\n[SUMMARY]")
    print(f"  Children updated with Enquiry ID: {updated_children}")
    print(f"  Enrollments updated: {updated_enrollments}")
...
def import_attendance_data(db: Session):
    ...
    print("\n" + "="*70)
    print("STEP 2: Importing Attendance Data from CSV")
    print("="*70)
    ...
    print(f"Found {len(rows)} attendance records in CSV")
    ...
    for row in rows:
        try:
            ...
            child_name = str(row.get('Child Name', '')).strip()
            batch_name = str(row.get('Batch', '')).strip()
            booked_classes = int(row.get('Booked Classes', 0)) if row.get('Booked Classes') else 24
            ...
            if not enrollment:
                print(f"  ! No enrollment for: {enquiry_id} - {child_name}")
                continue
            ...
            for att_date in attendance_dates:
                ...
                existing = db.query(Attendance).filter(
                    ...
                    Attendance.session_date == att_date
                ).first()
            ...
            enrollment.visits_used = attended_classes

            if attendance_dates:
                print(f"  ✓ {enquiry_id} - {child_name}: {len(attendance_dates)} attendance records")

        except Exception as e:
            print(f"  ! Error processing {enquiry_id}: {str(e)}")
            import traceback
            traceback.print_exc()
            continue
    ...
    print(f"  Children not found: {len(children_not_found)}")

    if children_not_found:
        print(f"\n  Missing children:")
        for nf in children_not_found[:10]:
            print(f"    - {nf}")
...
def verify_data(db: Session):
    ...
    print(f"  Children with Enquiry ID: {children_with_enquiry}")
    print(f"  Total enrollments: {enrollments}")
    print(f"  Total attendance records: {attendance}")

    if mahira:
        print(f"\n  Found Mahira:")
        print(f"    - ID: {mahira.id}")
        print(f"    - Enquiry ID: {mahira.enquiry_id}")
        print(f"    - Name: {mahira.first_name} {mahira.last_name or ''}")
        ...
        if enrollment:
            ...
            print(f"    - Booked: {enrollment.visits_included}")
            print(f"    - Used: {enrollment.visits_used}")
            print(f"    - Remaining: {enrollment.visits_included - enrollment.visits_used}")
...
def main():
    ...
    print("\n" + "="*70)
    print("IMPORTING CHANDIGARH DATA WITH ENQUIRY ID MAPPING")
    print("="*70)
